### Skrypt został zmodyfikowany, aby używać stabilnej wersji Schrodinger zamiast testowej Heisenberg:
### - Node pobierany z tagu: v0.2.10-matcha-shot
### - Miner pobierany z tagu: v0.3.0
### Dzięki temu skrypt działa z publicznie dostępną siecią testową.
### Pozostała konfiguracja (klucze, sieć, tmux, logi) bez zmian.
...
[ -d "$CHAIN_DIR/.git" ] || git clone -b v0.2.10-matcha-shot --depth 1 https://github.com/Quantus-Network/chain "$CHAIN_DIR"
[ -d "$CHAIN_DIR/quantus-miner/.git" ] || git clone -b v0.3.0 --depth 1 https://github.com/Quantus-Network/quantus-miner "$CHAIN_DIR/quantus-miner"
